...
class UserList:
    ...
    def get_completed_list(self) -> list[tuple[Book, Score]]:
        with self._db.client().cursor() as cur:
            cur.execute(
                f"SELECT bookid FROM bookstatus WHERE userid = %(userid)s AND status = 'completed' ORDER BY bookid ASC",
                {"userid": self.userid},
            )
    ...
    def get_planned_list(self) -> list[tuple[Book, Score]]:
        with self._db.client().cursor() as cur:
            cur.execute(
                f"SELECT bookid FROM bookstatus WHERE userid = %(userid)s AND status = 'planned' ORDER BY bookid ASC",
                {"userid": self.userid},
            )
    ...
    def get_reading_list(self) -> list[tuple[Book, Score]]:
        with self._db.client().cursor() as cur:
            cur.execute(
                f"SELECT bookid FROM bookstatus WHERE userid = %(userid)s AND status = 'reading' ORDER BY bookid ASC",
                {"userid": self.userid},
            )
