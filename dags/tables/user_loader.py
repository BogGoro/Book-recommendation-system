...
class User(BaseModel):
    ...
    def from_dict(cls, data: Tuple[Any]) -> "User":
        ...
        Args:
            data: Tuple containing (id, username, createts)

        Returns:
            User: Validated user object
        """
        return cls(id=data[0], username=data[1], createts=data[2])
...
class UserDestRepository:
    ...
    def insert_batch(self, conn: ClickhouseClient, users: List[User]) -> None:
        ...
        data = [[user.id, user.username, user.createts] for user in users]

        conn.execute(
            """
            INSERT INTO User (id, username, createts) VALUES
            """,
            data,
        )
