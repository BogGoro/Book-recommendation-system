...
class BookObj(BaseModel):
    ...
    Attributes:
        ...
        createts: Timestamp of last update
    """
    ...
    def from_dict(cls, data: Tuple[Any]) -> "BookObj":
        ...
        Args:
            data: Tuple containing book data in order:
                  (id, title, author, year, imgurl, description, createts)
...
class BookOriginRepository:
    ...
    def list_book(
        ...
        with self._db.client().cursor() as cur:
            cur.execute(
                """
                SELECT id, title, author, year, imgurl, description, createts
                FROM book
...
class BookDestRepository:
    ...
    def insert_batch(self, conn: ClickhouseClient, books: List[BookObj]) -> None:
        ...
        conn.execute(
            """
            INSERT INTO Book (id, title, author, year, imgurl, description, createts) VALUES
            """,
            data,
        )
...
class BookLoader:
    ...
    def load_book(self) -> None:
        ...
        with self.ch_dest.connection() as conn:
            ...
            last_loaded_date = conn.execute("SELECT MAX(createts) FROM Book")[0][0]
