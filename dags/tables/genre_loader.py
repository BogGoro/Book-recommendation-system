...
class GenreDestinationRepository:
    ...
    def insert_batch(self, conn: ClickhouseClient, genres: List[Genre]) -> None:
        ...
        data = [[genre.id, genre.name, genre.createts] for genre in genres]

        conn.execute(
            """
            INSERT INTO Genre (id, name, createts) VALUES
            """,
            data,
        )
...
class GenreLoader:
    ...
    def load_genres(self) -> None:
        ...
        with self.ch_dest.connection() as conn:
            ...
            last_loaded = conn.execute("SELECT MAX(createts) FROM Genre")[0][0]
            if not last_loaded:
