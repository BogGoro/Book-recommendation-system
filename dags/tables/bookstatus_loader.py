from logging import Logger
from typing import List, Tuple, Any
from datetime import datetime
from clickhouse_driver import Client as ClickhouseClient
from pydantic import BaseModel
from lib.pg_connect import PgConnect
from lib.ch_connect import CHConnect


class BookStatusRecord(BaseModel):
    """
    Data model representing a user's book record.

    Attributes:
        userid: ID of the user
        bookid: ID of the book
        status: Status of the book
        createts: Timestamp of when this record was last created
    """

    userid: int
    bookid: int
    version: int
    status: str
    createts: datetime

    @classmethod
    def from_dict(cls, data: Tuple[Any]) -> "BookStatusRecord":
        """
        Create BookStatusRecord from database tuple.

        Args:
            data: Tuple containing (userid, bookid, version, status, createts)

        Returns:
            BookStatusRecord: Validated book status record
        """
        return cls(
            userid=data[0],
            bookid=data[1],
            version=data[2],
            status=data[3],
            createts=data[4],
        )


class BookStatusesRepository:
    """
    Repository for retrieving book status records from PostgreSQL.
    Handles batched fetching with incremental loading support.
    """

    def __init__(self, pg: PgConnect) -> None:
        """
        Initialize with PostgreSQL connection.

        Args:
            pg: Configured PostgreSQL connection wrapper
        """
        self._db = pg

    def get_max_version(self) -> int:
        """
        Get the maximum version of the book rating table.

        Returns:
            int: Maximum version
        """
        with self._db.client().cursor() as cur:
            cur.execute(
                """
                SELECT MAX(version) 
                FROM bookstatus
                """
            )
            result = cur.fetchone()
            return result[0] if result else -1

    def list_book_statuses(
        self, threshold: int, target: int, batch_size: int = 10000
    ) -> List[BookStatusRecord]:
        """
        Get batch of book status records updated after threshold.

        Args:
            threshold: Minimum version to include
            target: Maximum version to include
            batch_size: Number of records per batch (default: 10,000)

        Returns:
            List[BookStatusRecord]: Batch of book status records
        """
        with self._db.client().cursor() as cur:
            cur.execute(
                """
                SELECT userid, bookid, version, status, createts
                FROM bookstatus
                WHERE version > %(threshold)s AND version <= %(target)s
                ORDER BY version ASC
                LIMIT %(batch_size)s
                """,
                {"threshold": threshold, "target": target, "batch_size": batch_size},
            )
            rows = cur.fetchall()
        return [BookStatusRecord.from_dict(row) for row in rows]


class BookStatusesDestRepository:
    """
    Repository for loading book status records into ClickHouse.
    Optimized for efficient batch inserts.
    """

    def insert_batch(
        self, conn: ClickhouseClient, records: List[BookStatusRecord]
    ) -> None:
        """
        Insert batch of book status records into ClickHouse.

        Args:
            conn: Active ClickHouse connection
            records: List of book status records to insert

        Note:
            Silently returns if input list is empty
        """
        if not records:
            return

        # Convert to ClickHouse-compatible format
        data = [
            [rec.userid, rec.bookid, rec.version, rec.status, rec.createts] for rec in records
        ]

        conn.execute(
            """
            INSERT INTO BookStatus (userid, bookid, version, status, createts) VALUES
            """,
            data,
        )


class BookStatusesLoader:
    """
    Orchestrates the complete ETL process for book status records.
    Implements incremental loading with progress tracking.
    """

    BATCH_SIZE = 10000  # Optimal batch size for bulk operations

    def __init__(self, pg_origin: PgConnect, ch_dest: CHConnect, log: Logger) -> None:
        """
        Initialize loader with connections and logger.

        Args:
            pg_origin: Source PostgreSQL connection
            ch_dest: Target ClickHouse connection
            log: Logger instance for progress tracking
        """
        self.ch_dest = ch_dest
        self.origin = BookStatusesRepository(pg_origin)
        self.stg = BookStatusesDestRepository()
        self.log = log

    def load_book_statuses(self) -> None:
        """
        Execute complete loading process:
        1. Gets last version from target
        2. Fetches batches from source updated since last load
        3. Inserts batches into target
        4. Repeats until all updates processed
        5. Logs progress and completion
        """
        with self.ch_dest.connection() as conn:
            # Get most recent update from target
            last_loaded = conn.execute("SELECT MAX(version) FROM BookStatus")[0][0]
            if not last_loaded:
                last_loaded = -1  # Initial load marker

            target_version = self.origin.get_max_version()

            if target_version <= last_loaded:
                self.log.info("No new book status records to load")
                return
            
            total_loaded = 0
            batch_num = 1

            # Process batches until completion
            while last_loaded != target_version:
                batch = self.origin.list_book_statuses(last_loaded, target_version, self.BATCH_SIZE)
                self.log.info(f"Batch {batch_num}: {len(batch)} records to load")
                if not batch:
                    break
                
                self.stg.insert_batch(conn, batch)

                # Update counters and get next batch
                total_loaded += len(batch)
                last_loaded = batch[-1].version
                batch_num += 1

            # Optimize table
            conn.execute("OPTIMIZE TABLE BookStatus FINAL")

            self.log.info(
                f"Load complete. Total book status records loaded: {total_loaded}"
            )
