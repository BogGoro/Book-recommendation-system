...
class BookType(BaseModel):
    ...
    def from_dict(cls, data: Tuple[Any]) -> "BookType":
        ...
        Args:
            data: Tuple containing (id, name, createts)

        Returns:
            BookType: Validated book type object
        """
        return cls(id=data[0], name=data[1], createts=data[2])
...
class BookTypeRepository:
    ...
    def list_book_types(
        ...
        with self._db.client().cursor() as cur:
            cur.execute(
                """
                SELECT id, name, createts
                FROM type
                WHERE createts > %(threshold)s
                ORDER BY createts ASC, id ASC
                LIMIT %(batch_size)s
                OFFSET %(offset)s
                """,
...
class BookTypeDestRepository:
    ...
    def insert_batch(self, conn: ClickhouseClient, types: List[BookType]) -> None:
        ...
        data = [
            [book_type.id, book_type.name, book_type.createts] for book_type in types
        ]

        conn.execute(
            """
            INSERT INTO Type (id, name, createts) VALUES
            """,
            data,
        )
...
class BookTypeLoader:
    ...
    def load_book_types(self) -> None:
        ...
        with self.ch_dest.connection() as conn:
            ...
            last_loaded = conn.execute("SELECT MAX(createts) FROM Type")[0][0]
            if not last_loaded:
