...
class BookTagObj(BaseModel):
    ...
    def from_dict(cls, data: Tuple[Any]) -> "BookTagObj":
        ...
        Args:
            data: Tuple containing (bookid, tagid, createts)

        Returns:
            BookTagObj: Validated relationship object
        """
        return cls(bookid=data[0], tagid=data[1], createts=data[2])
...
class BookTagOriginRepository:
    ...
    def list_book_tags(
        ...
        with self._db.client().cursor() as cur:
            cur.execute(
                """
                SELECT bookid, tagid, createts
                FROM booktag
                WHERE createts > %(threshold)s
                ORDER BY createts ASC, bookid ASC, tagid ASC
                LIMIT %(batch_size)s
                OFFSET %(offset)s
                """,
...
class BookTagDestRepository:
    ...
    def insert_batch(
        ...
        data = [[rel.bookid, rel.tagid, rel.createts] for rel in relationships]

        conn.execute(
            """
            INSERT INTO BookTag (bookid, tagid, createts) VALUES
            """,
            data,
        )
...
class BookTagLoader:
    ...
    def load_book_tags(self) -> None:
        ...
        with self.ch_dest.connection() as conn:
            ...
            last_loaded = conn.execute("SELECT MAX(createts) FROM BookTag")[0][0]
            if not last_loaded:
