...
class BookTypeObj(BaseModel):
    ...
    Attributes:
        ...
        createts: Timestamp of last update (for incremental loading)
    """
    ...
    def from_dict(cls, data: Tuple[Any]) -> "BookTypeObj":
        ...
        Args:
            data: Tuple containing (bookid, typeid, createts)

        Returns:
            BookTypeObj: Validated relationship object
        """
        return cls(bookid=data[0], typeid=data[1], createts=data[2])
...
class BookTypeOriginRepository:
    ...
    def list_book_types(
        ...
        with self._db.client().cursor() as cur:
            cur.execute(
                """
                SELECT bookid, typeid, createts
                FROM booktype
                WHERE createts > %(threshold)s
                ORDER BY createts ASC, bookid ASC, typeid ASC
                LIMIT %(batch_size)s
                OFFSET %(offset)s
                """,
...
class BookTypeDestRepository:
    ...
    def insert_batch(
        ...
        data = [[rel.bookid, rel.typeid, rel.createts] for rel in relationships]

        conn.execute(
            """
            INSERT INTO BookType (bookid, typeid, createts) VALUES
            """,
            data,
        )
...
class BookTypeLoader:
    ...
    def load_book_types(self) -> None:
        ...
        with self.ch_dest.connection() as conn:
            ...
            last_loaded = conn.execute("SELECT MAX(createts) FROM BookType")[0][0]
            if not last_loaded:
