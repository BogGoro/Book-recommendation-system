...
class BookGenreObj(BaseModel):
    ...
    Attributes:
        ...
        createts: Timestamp of last update (used for incremental loading)
    """
    ...
    def from_dict(cls, data: Tuple[Any]) -> "BookGenreObj":
        ...
        Args:
            data: Tuple containing (bookid, genreid, createts)

        Returns:
            BookGenreObj: Validated relationship object
        """
        return cls(bookid=data[0], genreid=data[1], createts=data[2])
...
class BookGenreOriginRepository:
    ...
    def list_book_genre(
        ...
        with self._db.client().cursor() as cur:
            cur.execute(
                """
                SELECT bookid, genreid, createts
                FROM bookgenre
                WHERE createts > %(threshold)s
                ORDER BY createts ASC, bookid ASC, genreid ASC
                LIMIT %(batch_size)s
                OFFSET %(offset)s
                """,
...
class BookGenreDestRepository:
    ...
    def insert_batch(
        ...
        data = [[rel.bookid, rel.genreid, rel.createts] for rel in relationships]

        conn.execute(
            """
            INSERT INTO BookGenre (bookid, genreid, createts) VALUES
            """,
            data,
        )
...
class BookGenreLoader:
    ...
    def load_book_genre(self) -> None:
        ...
        with self.ch_dest.connection() as conn:
            ...
            last_loaded = conn.execute("SELECT MAX(createts) FROM BookGenre")[0][0]
            if not last_loaded:
